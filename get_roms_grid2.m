function Gout = get_roms_grid2(Ginp, Sinp, Tindex)

% Modification of get_roms_grid that is part of
% roms-matlab/grid/get_roms_grid.m to handle the situation where zeta is in
% a separate NetCDF file from the rest of the internal variables. Correct
% usage: create a cell array for Sinp that contains both paths.
%
...
% svn $Id: get_roms_grid.m 1102 2022-01-06 21:50:44Z arango $
%=========================================================================%
...
if (nargin > 1)
  if (iscell(Sinp))
    process.zeta = true;
    Gout.his_name = Sinp{1};                    % Sinp is a NetCDF file
...
if (process.zeta)
    if (iscell(Sinp))
        for iHis=1:length(Sinp)
            Sinfo = nc_inq(Sinp{iHis});
            ...
            for n = 1:NvarsS
                ...
                switch field
                    case 'Vtransform'
                        Gout.(field) = nc_read(Sinp{iHis},field);
                        got.(field)  = true;
                    case 'Vstretching'
                        Gout.(field) = nc_read(Sinp{iHis},field);
                        got.(field)  = true;
